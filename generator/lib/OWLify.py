...
class OWL:
    ...
    def add_instance(self, instance_name, class_name):
        inst_decl = '<Declaration>\n'
        inst_decl += '\t<NamedIndividual IRI="#'+instance_name+'"/>\n'
        inst_decl += '</Declaration>'
        inst_instant = '<NamedIndividual IRI="#'+instance_name+'"/>'
        return

    def add_sub_class(self, classname, superclassname):
        stub = '<SubClassOf>\n'
        stub += '\t<Class IRI=\"' + self.namespace + '#' + classname + '\"/>\n'
    ...
    def assert_sub_class_triple(self, classname, propertyname, propertyvalue, relation):
        if relation == 'some':
    ...
    def assert_equivalent_triple(self, classname, propertyname, propertyvalue, relation):
        if relation == 'some':
            relstart = '\t<ObjectSomeValuesFrom>\n'
            relend = '\t</ObjectSomeValuesFrom>\n'
        else:
    ...
    def add_annotation(self, classname, message):
